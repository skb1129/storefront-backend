...
api.get("/all/", auth.requiresAuth(async (req, res, auth) => {
  if (!auth.admin) res.status(401).send();
  const orders = await Order.getAll();
  res.send(orders?.map((order) => order.getObject()));
}));

api.get("/:id/", auth.requiresAuth(async (req, res, auth) => {
  if (!auth.admin) res.status(401).send();
...
api.get("/user/:user_id/", auth.requiresAuth(async (req, res, auth) => {
  if (!auth.admin) res.status(401).send();
...
api.get("/", auth.requiresAuth(async (req, res, auth) => {
